...
import { LoginInput, queries, SendResetPasswordEmailInput, SignupInput } from '../../graphql';
...
export const authenticate = createAsyncThunk<AuthenticateReturned, AuthenticateThunkArg, AuthenticateThunkConfig>(
  'auth/authenticate',
  async (args, thunk) => {
    const { data, errors } = await queries.whoami.fetch();
...
export const login = createAsyncThunk<LoginReturned, LoginThunkArg, LoginThunkConfig>(
  'auth/login',
  async (args, thunk) => {
    const { data, errors } = await queries.login.fetch({ input: args.input });
    if (errors) {
      return thunk.rejectWithValue({ errors: errors.map((error) => error.message) });
    }
    if (!data || !data.login) {
...
export const signup = createAsyncThunk<SignupReturned, SignupThunkArg, SignupThunkConfig>(
  'auth/signup',
  async (args, thunk) => {
    const { data, errors } = await queries.signup.fetch({ input: args.input });
    if (errors) {
      return thunk.rejectWithValue({ errors: errors.map((error) => error.message) });
    }
    if (!data) {
...
export const logout = createAsyncThunk<LogoutReturned, LogoutThunkArg, LogoutThunkConfig>(
  'auth/logout',
  async (_, thunk) => {
    const { data, errors } = await queries.logout.fetch();
    if (errors) {
      return thunk.rejectWithValue({ errors: errors.map((error) => error.message) });
    }
    if (!data) {
...
>('auth/sendResetPasswordEmail', async (args, thunk) => {
  const { data, errors } = await queries.sendResetPasswordEmail.fetch({ input: args.input });
  if (errors) {
    return thunk.rejectWithValue({ errors: errors.map((error) => error.message) });
  }
  if (!data) {
