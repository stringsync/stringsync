...
export const ConfirmEmail: React.FC = enhance(() => {
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      const { data, errors } = await authClient.confirmEmail({ confirmationToken });
      if (errors) {
  ...
  const onResendConfirmationTokenClick = async () => {
    ...
    try {
      const authClient = AuthClient.create();
      const { data, errors } = await authClient.resendConfirmationEmail();
      if (errors) {
        setErrors(errors.map((error) => error.message));
        return;
      }
      ...
      <FormPage
        ...
        footer={
          <Row justify="center" align="middle">
            <Button block type="default" onClick={onResendConfirmationTokenClick} disabled={disabled}>
