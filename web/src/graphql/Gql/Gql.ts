...
export type RootOf<G extends AnyGql> = G extends Gql<infer T, any, any, any> ? T : never;
export type FieldOf<G extends AnyGql> = G extends Gql<any, infer F, any, any> ? F : never;
export type DataOf<G extends AnyGql> = G extends Gql<any, any, infer Q, any> ? Q : never;
export type VariablesOf<G extends AnyGql> = G extends Gql<any, any, any, infer V> ? V : never;

export type SuccessfulResponse<T extends Root, N extends Fields<T>> = { data: OnlyKey<N, T[N]>; errors?: never };
export type FailedResponse = { data: null; errors: GraphQLError[] };
export type GraphqlResponseOf<G extends AnyGql> = SuccessfulResponse<RootOf<G>, FieldOf<G>> | FailedResponse;
