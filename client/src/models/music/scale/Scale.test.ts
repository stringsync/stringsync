...
test('Scale.prototype.notes', () => {
  ...
  cases.forEach(testCase => {
    ...
    const scale = Scale.for(key as string, scaleName as string);

    const expectedNotes = (expectedNoteLiterals as string[]).map(literal => new Note(literal, 1));
...
test('Scale.prototype.notes accepts an array of octaves', () => {
  ...
  cases.forEach(testCase => {
    const [args, octaves, expectedNoteStrings] = testCase;
    const scale = Scale.for(args[0] as string, args[1] as string);
    const noteStrings = scale.notes(octaves as number[]).map(note => note.toString());
