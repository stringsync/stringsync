...
test('Note.prototype.isEquivalent', () => {
  ...
  cases.forEach(args => {
    ...
    const note1 = new Note(args1[0], args1[1]);
    const note2 = new Note(args2[0], args2[1]);
...
test('Note.prototype.toString', () => {
  ...
  cases.forEach(args => {
    const note = new Note(args[0] as string, args[1] as number);
...
test('Note.prototype.step', () => {
  ...
  cases.forEach(testCase => {
    ...
    const note = new Note(args[0], args[1]);
    const steppedNote = note.step(numHalfSteps as number);
    const expectedNote = new Note(expectedArgs[0], expectedArgs[1]);
...
test('Note.prototype.step numHalfSteps parameter defaults to 1', () => {
  ...
  cases.forEach((args, ndx) => {
    ...
    const note = new Note(literal as string, octave as number);
    const steppedNote = note.step();
    expect(steppedNote).not.toBe(note);

    const expectedLiteral = literals[(ndx + 1) % literals.length];
    const expectedOctave = (octave as number) + (ndx === (cases.length - 1) ? 1 : 0);
