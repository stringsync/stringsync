...
func TestRoute_Match(t *testing.T) {
	...
	for _, test := range []struct {
	...
	}{
		{
			...
			path:      "/bar",
			wantOk:    true,
			wantMatch: RouteMatch{Params{map[string]string{"foo": "bar"}}},
		}, {
			...
			path:      "/foo/bar/1",
			wantOk:    true,
			wantMatch: RouteMatch{Params{map[string]string{"id": "1"}}},
		}, {
			...
			path:      "/foo/abc/bar/1",
			wantOk:    true,
			wantMatch: RouteMatch{Params{map[string]string{"fooId": "abc", "barId": "1"}}},
		}, {
